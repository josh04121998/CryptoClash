...
**Not yet built:** crafting (spend duplicate `card_instances` — spec.md Section 18), a real Coins earn loop beyond the one-time welcome bonus, editions/foils/serials actually being granted (the collection screen and schema are ready for them; nothing produces them yet). See Section 7.
...
**Just landed (session 5):** Coins ledger + Packs + Collection screen — the whole off-chain collectibility loop from spec.md Sections 17/19/21 is now live end-to-end: earn Coins (welcome bonus only, for now) → open a pack (seeded/logged server RNG) → watch a reveal → see the pull reflected immediately in a real "digital binder" screen (owned/missing by faction/rarity) and in the ownership-gated deck builder. `grantStartingCollection` is narrowed to Commons-only in the same session, so Uncommon-and-above is genuinely pack-only — rarity (session 4) finally means something. Verified against a live local Postgres (migration applied, full 42-test suite green) and end-to-end in a real Chromium browser via a scripted wallet for both Packs and Collection (Playwright + `ethers`, same technique as session 4 — the Claude-in-Chrome extension wasn't connected this session either). The `0003` migration has also been applied to production and verified there (Section 4).
