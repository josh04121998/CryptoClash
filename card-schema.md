...
- **No card editions/rarity/collectibility** — that's the Gameplay Identity vs. Collectible Identity layer, now specced in full in `collectibility.md` (supersedes spec.md Sections 12-20's original write-up), which lives one level up from this document and isn't started; see architecture.md Section 6 for how that's meant to attach to a `CardTemplate` once it exists (`card_editions` / `card_instances` tables, template stays the single source of gameplay truth).
