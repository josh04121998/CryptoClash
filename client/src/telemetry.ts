...
// Re-exported so call sites and tests keep importing telemetry names from this
// module rather than reaching past it into the protocol package. The server
// holds the authoritative copy and is the real gate (never trust the client);
// sharing the list just makes a typo at a call site a TypeScript error instead
// of an event that vanishes silently on ingest.
export { isTelemetryEventName, isValidAnonId, TELEMETRY_EVENT_NAMES, TELEMETRY_MAX_BATCH_SIZE };
export type { TelemetryEventName, TelemetryProps };
