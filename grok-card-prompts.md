...
- **No ornamentation.** No corner brackets, no metal bevel/texture, no glow, no icons — a single flat-colored border stroke, rounded corners, nothing else. This is a deliberate simplification, not a placeholder; don't add detail back in without discussing it first.
...
### Common frame (`#9ca3af`)
> A clean, minimalist trading-card border, filling almost the entire canvas edge to edge with only a small rounded outer margin (~2–3% of the width) — a modern, Pokémon-style card border, not a fantasy RPG frame. A single flat-colored border stroke in muted silver-grey (`#9ca3af`), roughly 3% of the card's width thick, rounded corners. No corner ornaments, no metal bevel or texture, no glow, no gem sockets, no icons. Exactly two bordered boxes stacked vertically, separated by a plain gap in the same border color, nothing else inside the frame: (1) a large box at the top, ~56% of the card height, for the illustration; (2) a second box below it, ~36% of the card height, for card text. Both boxes are a plain flat dark fill (color unimportant — black is fine), no gradient, no texture. No text, no card name, no numbers, no character, no scene. Portrait orientation, roughly 2:3 aspect ratio.

### Uncommon frame (`#22c55e`)
> Same minimalist two-box structure as the Common tier — thin flat border ~3% of the width, rounded corners, no ornamentation, art box ~56% height, description box ~36% height, plain flat dark box fills — but in terminal-green (`#22c55e`). No text, no card name, no numbers, no character, no scene. Portrait orientation, roughly 2:3 aspect ratio.

### Rare frame (`#3b82f6`)
> Same minimalist two-box structure again — thin flat border ~3% of the width, rounded corners, no ornamentation, art box ~56% height, description box ~36% height, plain flat dark box fills — but in blue (`#3b82f6`). No text, no card name, no numbers, no character, no scene. Portrait orientation, roughly 2:3 aspect ratio.

### Epic frame (`#a855f7`)
> Same minimalist two-box structure again — thin flat border ~3% of the width, rounded corners, no ornamentation, art box ~56% height, description box ~36% height, plain flat dark box fills — but in purple (`#a855f7`). No text, no card name, no numbers, no character, no scene. Portrait orientation, roughly 2:3 aspect ratio.

### Legendary frame (`#f59e0b`)
> Same minimalist two-box structure again — thin flat border ~3% of the width, rounded corners, no ornamentation, art box ~56% height, description box ~36% height, plain flat dark box fills — but in gold (`#f59e0b`). No text, no card name, no numbers, no character, no scene. Portrait orientation, roughly 2:3 aspect ratio.
