...
Both fixes are folded into the per-tier prompts below. Before accepting any result this time: check the returned image is exactly 750×1050px, and sample a pixel from the center of the art window and the top-left corner — both should read at or extremely close to `#0b120e` (`11,18,14`). If either check fails, regenerate rather than accepting.

All five: exactly 750×1050px, no name, no creature, no card-specific content.

### Common frame (`#9ca3af`)
> An ornate card border/bezel: corner ornaments, a top ticker-badge frame, a name-plate band, and two stat-gem sockets (a hexagon socket for cost/attack, a circle socket for health). Muted silver-grey metal and gem tone (`#9ca3af`), restrained and simple ornamentation befitting the lowest rarity tier. Dark trading-floor/gold-green brand visual language otherwise. The art-window and every other background area must be a single perfectly flat, unlit solid fill of exactly `#0b120e` — no gradient, no vignette, no glow, no light spill from the frame into this area, like a solid chroma-key plate. No scene, no character, no text, no card name. Output exactly 750×1050px, portrait, nothing cropped or padded beyond that.

### Uncommon frame (`#22c55e`)
> Same frame structure as the Common tier — corner ornaments, top ticker-badge frame, name-plate band, hexagon cost/attack gem socket, circle health gem socket — but in terminal-green metal/gem tones (`#22c55e`), with slightly more detailed ornamentation than Common. The art-window and every other background area must be a single perfectly flat, unlit solid fill of exactly `#0b120e` — no gradient, no vignette, no glow, no light spill from the frame into this area, like a solid chroma-key plate. No scene, no text, no card name. Output exactly 750×1050px, portrait, nothing cropped or padded beyond that.

### Rare frame (`#3b82f6`)
> Same frame structure again, in blue metal/gem tones (`#3b82f6`), with noticeably more ornate metalwork than Uncommon. The art-window and every other background area must be a single perfectly flat, unlit solid fill of exactly `#0b120e` — no gradient, no vignette, no glow, no light spill from the frame into this area, like a solid chroma-key plate. No scene, no text, no card name. Output exactly 750×1050px, portrait, nothing cropped or padded beyond that.

### Epic frame (`#a855f7`)
> Same frame structure again, in purple metal/gem tones (`#a855f7`), with elaborate, richly detailed metal and gem work. The art-window and every other background area must be a single perfectly flat, unlit solid fill of exactly `#0b120e` — no gradient, no vignette, no glow, no light spill from the frame into this area, like a solid chroma-key plate. No scene, no text, no card name. Output exactly 750×1050px, portrait, nothing cropped or padded beyond that.

### Legendary frame (`#f59e0b`)
> Same frame structure again, in gold metal/gem tones (`#f59e0b`) — the most ornate and precious-looking of the five: heavy gold metalwork, glowing gem sockets, should read as a genuine treasure, the way Hearthstone's own Legendary frame does. The art-window and every other background area must be a single perfectly flat, unlit solid fill of exactly `#0b120e` — no gradient, no vignette, no glow, no light spill from the frame into this area, like a solid chroma-key plate. No scene, no text, no card name. Output exactly 750×1050px, portrait, nothing cropped or padded beyond that.
