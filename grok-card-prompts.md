...
**v1.3 correction (2026-09-14) — locked in.** The accepted reference frame (a Common-tier regen, not yet saved into `branding/assets/` — the Common prompt below describes it, and is now the template every other tier copies) fixed a fourth problem and is otherwise good enough to build the full five-tier set from:

- **Gem sockets were a flat diamond-outline icon with stray connector lines/wires poking out toward the card center** — read as broken/unfinished. Fixed by asking for a real dimensional faceted gem (beveled rim, specular highlight, actual facets) that fully fills its socket with nothing extending out of it. Now folded into all five prompts below.
- **Exact-pixel output is not something Grok honors** — every accepted frame across both regen rounds landed at a different pixel size (1152×1728, 1184×1664, ...) despite the prompt demanding 750×1050 each time. Stop fighting this in the prompt: generate at whatever size/ratio the tool gives you (a portrait preset close to 2:3 is fine), and resize/crop to exactly 750×1050 as a code step in the compositor instead. The dimension line below reflects this — it's no longer a hard prompt requirement.
- **The rules-text panel is still landing short of the 25% target** (~17–20% in accepted results, not the full quarter) despite asking directly. Not worth more regen cycles chasing it — accept ~20% as the realistic ceiling for this generator and handle the shortfall downstream (smaller rules-text font, tighter copy) rather than in the prompt.

Before accepting any result: sample a pixel from the art-window center, the recessed text-panel center, and a corner — all three should be near-identical to each other (uniformity matters more than hitting `#0b120e` exactly) and reasonably close to `#0b120e`/`11,18,14`. Check the gem sockets read as solid dimensional gems with nothing extending outside their socket. Don't reject a result solely for landing at the "wrong" pixel size — that resize happens later.

All five: same style/layout as the Common prompt below, no name, no creature, no card-specific content. Generate at a portrait aspect ratio (2:3 is a safe default); resize to exactly 750×1050px in the compositor after acceptance, not in the prompt.

### Common frame (`#9ca3af`)
> An ornate card border/bezel, laid out top to bottom as four distinct regions: (1) a top name-plate band, ~8% of total card height; (2) a large art window below it, ~55% of the card height; (3) a second recessed inset panel below the art window, clearly sized to at least 25% of the total card height — same sunken, bordered visual treatment as the name-plate band, not smaller than what a quarter of the card looks like; (4) a bottom footer strip in the remaining ~10% of height containing two stat-gem sockets. Each gem socket must be a solid, self-contained, dimensional faceted gem — a hexagon-cut gem on the left (cost/attack), a round-cut gem on the right (health) — beveled metal rim, natural stone facets, one bright specular highlight, sitting entirely inside its own socket. No thin connector lines, no wires or stubs extending out of the sockets toward the card center, no flat diamond-outline icon floating inside — just a real gem, fully filling its socket. Ornate bat-wing corner brackets framing all four corners. Muted silver-grey metal and gem tone (`#9ca3af`), restrained ornamentation for this, the lowest rarity tier. The art window, the recessed text panel, and every other background area must be a single perfectly flat, unlit solid fill of exactly `#0b120e` — no gradient, no vignette, no glow, no light spill from the frame into these areas. No scene, no character, no text, no card name. Portrait orientation.

### Uncommon frame (`#22c55e`)
> Same four-region structure and gem-socket treatment as the Common tier — name-plate band (~8% height), art window (~55% height), a recessed text panel below the art window at least 25% of the height, footer strip with a hexagon-cut cost/attack gem and a round-cut health gem, each a solid dimensional gem fully filling its socket with no connector lines or stubs extending out — but in terminal-green metal/gem tones (`#22c55e`), with slightly more detailed ornamentation than Common. Ornate bat-wing corner brackets. The art window, the recessed text panel, and every other background area must be a single perfectly flat, unlit solid fill of exactly `#0b120e` — no gradient, no vignette, no glow, no light spill. No scene, no text, no card name. Portrait orientation.

### Rare frame (`#3b82f6`)
> Same four-region structure and gem-socket treatment again — name-plate band (~8% height), art window (~55% height), a recessed text panel at least 25% of the height, footer strip with solid dimensional hexagon-cut/round-cut gems fully filling their sockets, no connector lines or stubs — in blue metal/gem tones (`#3b82f6`), with noticeably more ornate metalwork than Uncommon. Ornate bat-wing corner brackets. The art window, the recessed text panel, and every other background area must be a single perfectly flat, unlit solid fill of exactly `#0b120e` — no gradient, no vignette, no glow, no light spill. No scene, no text, no card name. Portrait orientation.

### Epic frame (`#a855f7`)
> Same four-region structure and gem-socket treatment again — name-plate band (~8% height), art window (~55% height), a recessed text panel at least 25% of the height, footer strip with solid dimensional hexagon-cut/round-cut gems fully filling their sockets, no connector lines or stubs — in purple metal/gem tones (`#a855f7`), with elaborate, richly detailed metal and gem work. Ornate bat-wing corner brackets. The art window, the recessed text panel, and every other background area must be a single perfectly flat, unlit solid fill of exactly `#0b120e` — no gradient, no vignette, no glow, no light spill. No scene, no text, no card name. Portrait orientation.

### Legendary frame (`#f59e0b`)
> Same four-region structure and gem-socket treatment again — name-plate band (~8% height), art window (~55% height), a recessed text panel at least 25% of the height, footer strip with solid dimensional hexagon-cut/round-cut gems fully filling their sockets, no connector lines or stubs — in gold metal/gem tones (`#f59e0b`) — the most ornate and precious-looking of the five: heavy gold metalwork, glowing gem sockets, should read as a genuine treasure, the way Hearthstone's own Legendary frame does. Ornate bat-wing corner brackets. The art window, the recessed text panel, and every other background area must be a single perfectly flat, unlit solid fill of exactly `#0b120e` — no gradient, no vignette, no glow, no light spill. No scene, no text, no card name. Portrait orientation.
