...
>
> **Still not started:** quest/daily/achievement/ranked-progression earn events (Section 21's other sources), the crafting service (duplicates from packs sit in the collection as real rows today — Section 18's "duplicates have value" holds structurally — but nothing yet lets a player spend them), and First Edition/Genesis's actual grant mechanism (a time-boxed print-run flag and an event/achievement path, respectively — neither exists yet, both are deliberately kept out of the pack RNG in the meantime).
>
