...
3. **Condition rolls on every paid pull, fixed baseline on free ones (Section 7): recommend yes**, mirroring CS:GO's "everything has a wear value, even the cheapest items" precedent, since it's cheap to implement uniformly and creates fun surprises across the whole pool, not just premium prints. **Fully built (2026-09-17)** — schema (migration `0009`), the roll-at-grant logic (`packsRepo.ts`'s `rollCondition`, the Section 7 weight table), and cosmetic CSS presentation (`CardFace.tsx`/`styles.css`, a glossy sheen at grade 9-10 and a desaturated/scuffed look at grade 1-2) are all live. Crafting (`craftingRepo.ts`) treats itself like Starter Decks — a fixed baseline grade, no roll — since it's already deterministic/non-foil by design and a random grade would be the odd one out among its guarantees; that's a judgment call this doc didn't originally make explicit, noted here for the record.
